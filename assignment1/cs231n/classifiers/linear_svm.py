...
def svm_loss_naive(W, X, y, reg):
    ...
    for i in range(num_train):
        ...
        for j in range(num_classes):
            ...
            if margin > 0:
                loss += margin
                dW[:,j] += X[i].T      # loss for `$s_j$`
                dW[:,y[i]] += -X[i].T  # loss for `$s_{y_i}$`
...
def svm_loss_vectorized(W, X, y, reg):
  ...
  #   dW[:, k] = wj + wy
    ...
    X_mask = np.zeros_like(margins)
    X_mask[margins > 0] = 1
    incorrect_counts = np.sum(X_mask, axis=1) 
    X_mask[np.arange(num_train), y] = -incorrect_counts # loss count for `$s_{y_i}$`

    dW = (X.T).dot(X_mask)
    dW = dW/num_train + reg * W * 0.5
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
