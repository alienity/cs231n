...
class CaptioningRNN(object):
    ...
    def sample(self, features, max_length=30):
        ...
        if self.cell_type == 'lstm':
            cur_cell_state = np.zeros_like(hidden_state)
        
        # Embed the input word captions.        
        embed_word, _ = word_embedding_forward(self._start, W_embed)
        
        for i in range(max_length):
            # RNN one step forward
            if self.cell_type == 'rnn':
                hidden_state, _ = rnn_step_forward(embed_word, hidden_state, Wx, Wh, b)
            elif self.cell_type == 'lstm':
                hidden_state, cur_cell_state, _ = lstm_step_forward(embed_word, hidden_state, cur_cell_state, Wx, Wh, b)
            # Scores and loss.
            ...
            embed_word, _ = word_embedding_forward(captions[:, i], W_embed)
